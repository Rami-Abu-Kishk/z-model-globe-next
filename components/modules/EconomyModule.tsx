      ...
      >
        ...
        <div className="flex flex-col gap-2 mb-4">
          <div className="flex justify-between items-center">
            <span className="text-[12px] font-black text-slate-400 uppercase tracking-widest leading-none">Indicator Tracking</span>
            <span className={`text-[15px] font-black leading-none ${isPositive ? 'text-emerald-600' : 'text-rose-600'}`}>
              {isPositive ? '+' : ''}{trend.value}%
            </span>
          </div>
          <h5 className="text-[15px] font-black text-slate-900 uppercase tracking-tight leading-snug line-clamp-2 min-h-[2.5rem]">
            {trend.label}
          </h5>
        </div>
            ...
            <div className="p-10">
              <div className="flex justify-between items-start mb-8">
                <div className="space-y-1">
                  ...
                  <div className={`text-[12px] font-black uppercase tracking-[0.2em] pt-1 ${isPositive ? 'text-emerald-600' : 'text-rose-600'}`}>
              ...
              <div className="p-8 bg-indigo-50/20 backdrop-blur-sm border border-indigo-100/50 rounded-3xl mb-8 relative overflow-hidden group/reasoning">
                ...
                <div className="flex justify-between items-center mb-4">
                  <div className="flex items-center gap-2">
                    ...
                    <span className="text-[12px] font-black text-indigo-600 uppercase tracking-[0.2em]">AI Predictive Synthesis</span>
                  </div>
                  <Badge className="bg-indigo-500 text-white border-none text-[10px] font-black uppercase tracking-widest shadow-sm">Z-Model Core</Badge>
                </div>
              ...
              {trend.relatedCountries && trend.relatedCountries.length > 0 && (
                <div className="mb-6">
                  <h4 className="text-[12px] font-black text-slate-400 uppercase tracking-widest mb-3">Top Impacted Regions</h4>
                  <div className="flex flex-wrap gap-2">
                    {trend.relatedCountries.map(iso => (
                      <div key={iso} className="flex items-center gap-2 px-3 py-1.5 bg-white rounded-lg border border-slate-200 shadow-sm">
                        ...
                        <span className="text-[12px] font-black tracking-widest uppercase text-slate-700">{iso}</span>
                      </div>
                    ))}
                  </div>
                </div>
              ...
              <div className="mt-8 pt-6 border-t border-slate-100 flex justify-end">
                <button
                  ...
                  className="px-6 py-2 bg-slate-900 text-white text-[12px] font-black uppercase tracking-widest rounded-full hover:bg-slate-800 transition-colors cursor-pointer"
                >
                  Close Analysis
                </button>
              </div>
        ...
        <div className="absolute inset-0 p-6 backdrop-blur-2xl border border-white/60 bg-white/40 rounded-2xl shadow-xl hover:shadow-2xl flex flex-col transition-all duration-300 group-hover:bg-white/60">
          ...
          <div className="flex justify-between items-start mb-6 relative z-10">
            <div className="flex items-center gap-1.5 opacity-60">
              ...
              <span className="text-[11px] font-black text-slate-400 uppercase tracking-widest leading-none">Last Update: 2h ago</span>
            </div>
            {(kpi.org.toLowerCase().includes('world bank') || kpi.rep.toLowerCase().includes('world bank')) ? (
              ...
              <span className="text-[12px] font-black uppercase tracking-widest text-slate-400">{kpi.org}</span>
            )}
          </div>

          <div className="relative z-10">
            <h3 className="text-3xl font-black mb-2 tracking-tighter text-slate-900">{kpi.value}</h3>
            <p className="text-[14px] font-bold leading-snug uppercase tracking-tight transition-colors text-slate-600 group-hover:text-slate-900">
    ...
    >
      <div className="flex justify-between items-start mb-6">
        {(report.org.toLowerCase().includes('world bank') || report.author.toLowerCase().includes('world bank')) ? (
          ...
          <span className="px-2 py-0.5 rounded-full border border-slate-200 text-slate-500 text-[11px] font-black uppercase tracking-widest">{report.org}</span>
        )}
        ...
        <div className="flex flex-col gap-12">
          ...
          <div className="space-y-6">
            <div className="flex items-center gap-3 px-2 border-l-4 border-emerald-500">
              ...
              <div>
                <h4 className="text-[14px] font-black text-emerald-600 uppercase tracking-[0.2em] leading-none">Positive Trends</h4>
                <div className="flex items-center gap-2 mt-1">
                  <p className="text-[14px] text-slate-400 font-bold uppercase">Sectors currently exceeding historical CAGR benchmarks</p>
                  <span className="text-[12px] text-slate-300 font-black uppercase tracking-widest ml-2">• Last update: 1 hour ago</span>
                </div>
              </div>
            </div>
          ...
          <div className="space-y-6">
            <div className="flex items-center gap-3 px-2 border-l-4 border-rose-500">
              ...
              <div>
                <h4 className="text-[14px] font-black text-rose-600 uppercase tracking-[0.2em] leading-none">Negative Trends</h4>
                <div className="flex items-center gap-2 mt-1">
                  <p className="text-[14px] text-slate-400 font-bold uppercase">High-pressure indicators requiring strategic mediation</p>
                  <span className="text-[12px] text-slate-300 font-black uppercase tracking-widest ml-2">• Last update: 1 hour ago</span>
                </div>
              </div>
            </div>
        ...
        <div className="p-5 space-y-8">
          ...
          <div className="space-y-4">
            <div className="flex items-center justify-between px-1">
              <span className="text-[12px] font-black text-emerald-600 uppercase tracking-[0.1em]">Positive Trends</span>
          ...
          <div className="space-y-4">
            <div className="flex items-center justify-between px-1">
              <span className="text-[12px] font-black text-rose-600 uppercase tracking-[0.1em]">Negative Trends</span>
          ...
          <div className="space-y-4 pt-2">
            <div className="flex justify-between items-center px-1">
              <span className="text-[12px] font-black text-slate-400 uppercase tracking-[0.1em]">Institutional Pulse</span>
              <span className="text-[10px] text-slate-300 font-bold uppercase tracking-tighter">Updated 2h ago</span>
            </div>
          ...
          {data.reports && data.reports.length > 0 && (
            <div className="space-y-4 pt-2">
              <h4 className="text-[12px] font-black text-slate-400 uppercase tracking-widest px-1">Strategic Reports</h4>
                ...
                {data.reports.slice(0, 2).map((report, idx) => (
                  ...
                  >
                    <div className="flex justify-between items-start mb-1">
                      <span className="text-[13px] font-black text-slate-900 uppercase tracking-tight group-hover:text-emerald-700 transition-colors">{report.title}</span>
                    </div>
                    <div className="flex justify-between items-center">
                      <span className="text-[11px] text-slate-400 font-bold uppercase">{report.org}</span>
                      <span className="text-[11px] text-emerald-600 font-black uppercase tracking-tighter">View PDF</span>
                    </div>
                  </div>
                ))}
              </div>
